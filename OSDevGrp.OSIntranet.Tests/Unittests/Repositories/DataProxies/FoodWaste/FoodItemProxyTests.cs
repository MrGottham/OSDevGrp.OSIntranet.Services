...
namespace OSDevGrp.OSIntranet.Tests.Unittests.Repositories.DataProxies.FoodWaste
{
    ...
    public class FoodItemProxyTests
    {
        ...
        public void TestThatMapDataThrowsArgumentNullExceptionIfDataReaderIsNull()
        {
            ...
            var exception = Assert.Throws<ArgumentNullException>(() => foodItemProxy.MapData(null, fixture.Create<IMySqlDataProvider>()));
            Assert.That(exception, Is.Not.Null);
            Assert.That(exception.ParamName, Is.Not.Null);
            Assert.That(exception.ParamName, Is.Not.Empty);
        ...
        public void TestThatMapDataAndMapRelationsMapsDataIntoProxy(bool isActive)
        {
            ...
            dataProviderBaseMock.Stub(m => m.Clone())
                ...
                .Repeat.Any();
            dataProviderBaseMock.Stub(m => m.GetCollection<FoodItemGroupProxy>(Arg<MySqlCommand>.Is.Anything))
            ...
            dataProviderBaseMock.AssertWasCalled(m => m.GetCollection<FoodItemGroupProxy>(Arg<MySqlCommand>.Matches(cmd => cmd.CommandText == string.Format("SELECT FoodItemGroupIdentifier,FoodItemIdentifier,FoodGroupIdentifier,IsPrimary FROM FoodItemGroups WHERE FoodItemIdentifier='{0}'", dataReader.GetString("FoodItemIdentifier")))));
        ...
        public void TestThatSaveRelationsThrowsIntranetRepositoryExceptionWhenIdentifierIsNull()
        {
            ...
            var exception = Assert.Throws<IntranetRepositoryException>(() => foodItemProxy.SaveRelations(fixture.Create<IMySqlDataProvider>(), fixture.Create<bool>()));
            Assert.That(exception, Is.Not.Null);
            Assert.That(exception.Message, Is.Not.Null);
            Assert.That(exception.Message, Is.Not.Empty);
            Assert.That(exception.Message, Is.EqualTo(Resource.GetExceptionMessage(ExceptionMessage.IllegalValue, foodItemProxy.Identifier, "Identifier")));
        ...
        {
            var fixture = new Fixture();
            fixture.Customize<IFoodWasteDataProvider>(e => e.FromFactory(() => MockRepository.GenerateStub<IFoodWasteDataProvider>()));
            ...
            var exception = Assert.Throws<IntranetRepositoryException>(() => foodItemProxy.SaveRelations(fixture.Create<IFoodWasteDataProvider>(), fixture.Create<bool>()));
            Assert.That(exception, Is.Not.Null);
            Assert.That(exception.Message, Is.Not.Null);
            Assert.That(exception.Message, Is.Not.Empty);
        ...
        public void TestThatSaveRelationsInsertsMissingFoodItemGroups()
        {
            ...
            var dataProviderMock = MockRepository.GenerateMock<IFoodWasteDataProvider>();
            ...
            // ReSharper disable PossibleInvalidOperationException
            dataProviderMock.AssertWasCalled(m => m.GetCollection<FoodItemGroupProxy>(Arg<MySqlCommand>.Matches(cmd => cmd.CommandText == string.Format("SELECT FoodItemGroupIdentifier,FoodItemIdentifier,FoodGroupIdentifier,IsPrimary FROM FoodItemGroups WHERE FoodItemIdentifier='{0}'", foodItemProxy.Identifier.Value.ToString("D").ToUpper()))));
            // ReSharper restore PossibleInvalidOperationException
        ...
        public void TestThatDeleteRelationsThrowsIntranetRepositoryExceptionWhenIdentifierIsNull()
        {
            ...
            var exception = Assert.Throws<IntranetRepositoryException>(() => foodItemProxy.DeleteRelations(fixture.Create<IMySqlDataProvider>()));
            Assert.That(exception, Is.Not.Null);
            Assert.That(exception.Message, Is.Not.Null);
            Assert.That(exception.Message, Is.Not.Empty);
            Assert.That(exception.Message, Is.EqualTo(Resource.GetExceptionMessage(ExceptionMessage.IllegalValue, foodItemProxy.Identifier, "Identifier")));
        ...
        {
            var dataProviderMock = MockRepository.GenerateStub<IFoodWasteDataProvider>();
            dataProviderMock.Stub(m => m.Clone())
                .Return(dataProviderMock)
                .Repeat.Any();
            dataProviderMock.Stub(m => m.GetCollection<FoodItemGroupProxy>(Arg<MySqlCommand>.Is.Anything))
